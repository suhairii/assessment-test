...
import { Printer, Home, User, Briefcase, Calendar, Share } from "lucide-react";
    ...
    <div className="min-h-screen bg-white text-black font-sans p-4 md:p-12 print:p-0 print:bg-white print:min-h-0 print:h-auto print:overflow-visible">
      ...
      <div className="max-w-4xl mx-auto print:max-w-none print:w-full">
        ...
        <div className="mt-10 pt-6 border-t border-gray-100 text-center no-print flex flex-col md:flex-row items-center justify-center gap-4 pb-10">
            {isAdmin ? (
                <Link href="/admin/dashboard" className="flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm text-gray-500 hover:text-black hover:bg-gray-100 transition"><Home size={18}/> Kembali</Link>
            ) : (
                <p className="text-xs text-gray-400 italic">Laporan evaluasi otomatis.</p>
            )}
            <button onClick={() => setShareModalOpen(true)} className="w-full md:w-auto flex items-center justify-center gap-2 bg-gray-100 text-black px-8 py-3 rounded-full hover:bg-gray-200 transition font-bold text-sm">
